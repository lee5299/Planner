import {stateSchema,careAt,today,type State} from './model.ts';
...
export function prepareSave(previous:State,input:unknown,now=new Date()):State{
 ...
 for(const e of next.executions){if(!next.tasks.some(t=>t.id===e.taskId))throw Error('실행 기록의 업무가 없습니다');if(e.date>today(next.timezone,now))throw Error('미래 실행 기록을 저장할 수 없습니다');const old=previous.executions.find(x=>x.id===e.id);if(old&&JSON.stringify(old)!==JSON.stringify(e))throw Error('저장된 실행 기록은 변경할 수 없습니다');e.createdAt=old?.createdAt??stamp;if(e.start&&e.end){e.minutes=Math.round((Date.parse(e.end)-Date.parse(e.start))/60000*100)/100;}}
